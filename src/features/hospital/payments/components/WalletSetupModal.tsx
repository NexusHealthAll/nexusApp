...
export function WalletSetupModal({
  ...
  const isProvisioned = Boolean(wallet?.safehavenAccountNumber);
  ...
  const handleCopy = () => {
    if (!deposit) return;
    navigator.clipboard.writeText(deposit.virtualAccountNumber).then(() => {
      setCopied(true);
      ...
      {stage === "deposit-ready" && deposit && (
        <div className="flex flex-col gap-5">
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            ...
            to the account below. Your wallet is credited automatically once
            we receive it.
          </p>
          ...
          <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-800">
            ...
            <p className="mt-3 text-xs text-neutral-400 dark:text-neutral-500">
              Expires{" "}
              {new Date(deposit.expiresAt).toLocaleString("en-NG", {
                dateStyle: "medium",
                timeStyle: "short",
              })}
            </p>
